let demojs={
	"storymap": {
		"calculate_zoom": false,
		"attribution": "",
		"call_to_action": false,
		"show_history_line":false,
		"zoomify": false,
		"map_type": "https://mt1.google.com/vt/lyrs=y&hl=en&x={x}&y={y}&z={z}",
		"call_to_action_text": "",
		"map_as_image": false,
		"map_subdomains":"xyz",
		"slides": [
			{
			"background": {         
                    		"url": "images/IMG_9391_11zon.jpg",
                    		"color": "#464646",
                    		"opacity": 99					
                	},
			"date": "Jan 2006",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/i0bm3ewj691f3c8pydz37/badlands.mp4?rlkey=lvg59pkuo7hgte3x938pjlhaq&st=x41vb86n&raw=1",
				"caption": "",
				"credit": ""
			},
			"type": "overview",
			"location": {
				"line": false
			},
			"text": {
				"headline": "Badlands Trail<small>Laguna Niguel, CA</small>",
				"text": "This trail offers some of the best oceanside views in Orange County without requiring much effort<ul><b><li>1.5 miles</li><li>Out & Back</li><li>Ocean Views, No Shade</li></b></ul>"
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/badlands_park_parking_11zon.png",
				"caption": "",
				"credit": "PARK ON ISLE VISTA"
			},
			"location": { 
				"lat":  33.503905,
				"line": false,
				"lon": -117.732192,
				"zoom": 20
			},
			"text": {
				"headline": "Parking<br><small><u><a href='https://maps.app.goo.gl/YZHAAhFqjfUbBmGX9' target='_blank'>31626 Isle Vista Laguna Niguel, CA 92677↗</a></u></small>",
				"text": "Street parking on Isle Vista. Head towards Badlands Park after parking. Two ADA parking spots available directly next to park."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9423_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.5033921, 
				"line": false,
				"lon": -117.7320602,
				"zoom": 20
			},
			"text": {
				"headline": "Start Here",
				"text": "Once parked, go up these stairs."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/badlands_start_11zon.png",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.5031027,
				"line": false,
				"lon": -117.7321197,
				"zoom": 20
			},
			"text": {
				"headline": "Badlands Park",
				"text": "Once at the top, you are now within Badlands Park. Turn left."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9375_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.5028,
				"line": false,
				"lon": -117.7319,
				"zoom": 20
			},
			"text": {
				"headline": "Stay Straight",
				"text": "The ocean is to the right."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/w4ihp1hnuzzjoad84ko1z/IMG_9376.JPG?rlkey=xy1l0xl67m71q592uupg4dbtk&st=ovojthk8&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.5019941,
				"line": false,
				"lon": -117.7318337,
				"zoom": 20
			},
			"text": {
				"headline": "Turn Right",
				"text": "Take these stairs down into the \"Badlands\""
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9379_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.5019941,
				"line": false,
				"lon": -117.7318337,
				"zoom": 20
			},
			"text": {
				"headline": "Long Flight of Stairs",
				"text": "Take these stairs on the right side of the trail down into the \"Badlands\""
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9380_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.502173,
				"line": false,
				"lon": 	-117.732167,
				"zoom": 20
			},
			"text": {
				"headline": "Badlands Trail",
				"text": "A sign to let you know you are on the Badlands Trail. Follow the path."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9382_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.502053,
				"line": false,
				"lon": 	-117.732757,
				"zoom": 20
			},
			"text": {
				"headline": "Keep Left",
				"text": "Stay as far left as possible to stay on the trail. People may be to the right taking photos or relaxing."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9384_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.501941,
				"line": false,
				"lon": -117.732916,
				"zoom": 20
			},
			"text": {
				"headline": "End of County Trail",
				"text": "Eventually, a sign that reads <i>End of County Trail</i> appears. Continue past this. The trail switches from being county maintained to being managed by the local home owners association here."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/h31n7qsrnq6hycce3vjvq/badlands_2.mp4?rlkey=ahz9cppflnj0ndbqzou265qlt&st=hsdw348t&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.501710,
				"line": false,
				"lon": -117.733048,
				"zoom": 20
			},
			"text": {
				"headline": "End of County Trail",
				"text": "Keep going towards the sign seen in the distance."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/gl87uu7fn867qbm9te3vy/badlands_3.mp4?rlkey=daj7hkyhlzxm4to5chyj7zruf&st=buoxosym&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.501569,
				"line": false,
				"lon": 	-117.733033,
				"zoom": 20
			},
			"text": {
				"headline": "Clear Ocean View",
				"text": "Laguna Beach and the ocean are unobstructed the rest of the way, except if there are clouds."
			}
			}, 
			{
			"date": "",
			"media": {
				"url": "images/IMG_9391_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.5008678,
				"line": false,
				"lon": 	-117.7333878,
				"zoom": 20
			},
			"text": {
				"headline": "Enjoy the View",
				"text": "Clear unobstructed views of the ocean continued."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/i0bm3ewj691f3c8pydz37/badlands.mp4?rlkey=lvg59pkuo7hgte3x938pjlhaq&st=vu0eenod&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.4995,
				"line": false,
				"lon": 	-117.7336,
				"zoom": 20
			},
			"text": {
				"headline": "Enjoy the View",
				"text": "Use caution. No barriers exist to prevent falling down the slope."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/zubmjequ4ulw3m2v7axaw/badlands_4.mp4?rlkey=mqvx8652bq1o0d6d7g1rbdhy0&st=gqpakky9&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.4995,
				"line": false,
				"lon": 	-117.7336,
				"zoom": 20
			},
			"text": {
				"headline": "Enjoy the View",
				"text": "Use caution. No barriers exist to prevent falling down the slope."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/B5674BF1-5CD2-4470-9893-79B3A66F2B61_11zon.jpeg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.498170742455144, 
				"line": false,
				"lon": 	-117.7330402423234,
				"zoom": 20
			},
			"text": {
				"headline": "Veer Left",
				"text": "The trail will veer left away from the ocean."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9396_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.498513,
				"line": false,
				"lon": 	-117.733075,
				"zoom": 20
			},
			"text": {
				"headline": "Narrow Section",
				"text": "This area is the most narrow with the steepest slope, use utmost caution."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/xofe9ccdk93tk4kiosir3/IMG_9399.JPG?rlkey=dxnl13tjs2sac6c1vwcoxpid0&st=lg7u5bba&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.49924,
				"line": false,
				"lon": -117.733059,
				"zoom": 20
			},
			"text": {
				"headline": "Turn Right",
				"text": "Turn right here where this railing is. The ocean is now more distant."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/dzzlwdttr4409c23fym6e/badlands_5.mp4?rlkey=39nznjxjjgqun50xbko76mr5j&st=mud099au&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.49924,
				"line": false,
				"lon": -117.733059,
				"zoom": 20
			},
			"text": {
				"headline": "Turn Right",
				"text": "Turn right here where this railing is. The ocean is now more distant."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9404_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.499347,
				"line": false,
				"lon":	-117.731923,
				"zoom": 20
			},
			"text": {
				"headline": "Summit Homes",
				"text": "Walk alongside nice summit homes."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/a1fmjpfwm7ebhclobmssg/IMG_9415.JPG?rlkey=8k3t4yit3g5dbvxoikmgdrc4a&st=v1a2n72s&raw=1",
				"caption": "",
				"credit": "FINAL OVERLOOK"
			},
			"location": {
				"lat": 	33.4986,
				"line": false,
				"lon":	-117.7310,
				"zoom": 20
			},
			"text": {
				"headline": "Final View",
				"text": "This is the final view. A sign that reads <i>End of Trail</i> will be on the right. Enjoy the last view looking south into Dana Point and of the ocean before heading back."
			}
			}
		]
	}
  }
