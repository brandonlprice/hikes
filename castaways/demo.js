let demojs={
	"storymap": {
		"calculate_zoom": false,
		"attribution": "",
		"call_to_action": false,
		"show_history_line":false,
		"zoomify": false,
		"map_type": "http://mt0.google.com/vt/lyrs=y&hl=en&x={x}&y={y}&z={z}",
		"call_to_action_text": "",
		"map_as_image": false,
		"map_subdomains":"xyz",
		"slides": [
			{
			"date": "Jan 2006",
			"media": {
				"url": "images/topoftheworld.JPG",
				"caption": "<i>Castaways Park, Newport Beach, Summer 2022</i>",
				"credit": ""
			},
			"type": "overview",
			"location": {
				"line": false
			},
			"text": {
				"headline": "<h2 style='color: red;'>Castaways Park, Newport Beach</h2>",
				"text": "This is a nice and short trail everyone can do that takes you to viewpoints that overlook Newport Harbor and Newport Back Bay. There is also a veteran memorial statue and flag. Several United States flags and pictures of veterans are placed throughout the park each Memorial Day in their honor."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_6468_5_11zon.jpg",
				"caption": "Castaways Lane",
				"credit": ""
			},
			"location": { 
				"lat": 33.62192368235754, 
				"line": false,
				"lon": -117.9059037182933,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Getting There</h6><font style='line-height: 1em;' color=red size=6>Castaways Lane</font>",
				"text": "From Castaways Lane, turn right here into the parking lot when you see this sign."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_6466_4_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.621374797799, 
				"line": false,
				"lon": -117.90644150120042,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Start Here</h6> <small style='color:red;'><ul><li>.5 mile-loop with option to extend</li><li>Flat and open with ocean views</li><li>No Shade</li><li>ADA Accessible with paved pathways</li><li>Park at <a href='https://maps.app.goo.gl/VZaywRQzbwzkgw527' target='_blank'><i>798 Dover Dr Newport Beach, CA 92663↗</i></a></li></ul></small>",
				"text": "Once parked, start the hike here."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_6463_3_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.621071597613565,   
				"line": false,
				"lon": -117.90633622449629,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Go Straight</h6>",
				"text": "Continue on this pathway for a few hundred feet."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/38EBE061-152E-4BCE-8916-6B63EA4F74FA_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat":  33.62073656854489,  
				"line": false,
				"lon": -117.90636572879542,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Trail Sign</h6>",
				"text": "Go past this sign and onto the dirt path for a short distance."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/D5C0DB50-33C2-43FB-95A2-AE97FCDA5EE5_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.620475244963, 
				"line": false,
				"lon": -117.90640462082716,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Turn Right</h6>",
				"text": "Once you hit the paved path, go right."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/691EC4FD-92E3-4BC7-AB07-91E37440FA84_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.620273908463815,  
				"line": false,
				"lon": -117.90649512916823,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Turn Left</h6>",
				"text": "The path you are on will dead end here. Take a left at the dead end."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/91E9BC99-215D-42B0-B82F-8AF807FD2548.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.62011061905886, 
				"line": false,
				"lon": -117.90625441712183,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Stay Straight</h6>",
				"text": "You will stay on this path for awhile. No need to take any of the offshoot paths."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_6445_3_11zon.jpg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.618954191968314, 
				"line": false,
				"lon": -117.90595535081673,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Entering Rest Area</h6>",
				"text": "Stay right. Another path will join in from the left. This area overlooks Newport Harbor. There is one bench to rest at."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/055C9AB3-ABE2-4413-A4DB-D18AF08816C9_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.618525343989546, 
				"line": false,
				"lon": -117.90604118150512,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Newport Harbor</h6>",
				"text": "The path starts to loop back in the other direction. Look to the right to see the harbor. A permanent veteran memorial can be seen in the distance."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/DB1330DA-A372-4D0B-8598-A282667A7732_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.61910549004932,  
				"line": false,
				"lon": -117.90557246894416,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Newport Back Bay</h6>",
				"text": "The same path you were on has now looped back in the opposite direction with views of the bay to the right."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/FBC64B7A-3BAC-4374-B3A1-40B4AF1BC0F6_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.61986101974898,  
				"line": false,
				"lon": -117.90516946357602,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Heading Back</h6>",
				"text": "Turn left here to head back. (An option to extend involves going straight. The path will eventually veer left and run into Dover Dr. Turn left there and take the sidewalk back to the parking lot.)"
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/E5749EF4-B5C1-479B-BA4C-EA3FB6927EF6_11zon.jpeg",
				"caption": "<i></i>",
				"credit": ""
			},
			"location": {
				"lat": 33.62062489188625, 
				"line": false,
				"lon": -117.90612969440221,
				"zoom": 19
			},
			"text": {
				"headline": "<h6 style='color:red;'>Pass the sign</h6>",
				"text": "Go past the trail sign on the left back to the parking lot."
			}
			}
		]
	}
  }
