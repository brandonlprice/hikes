...
(() => {
	...
	(() => {
		...
		class ot extends V {
			...
			_createTileLayer(t, e) {
				...
				switch (n[0]) {
					case "mapbox":
						var s;
						if (e.attribution = o + "<div class='mapbox-maplogo'></div><a href='https://www.mapbox.com/about/maps/' target='_blank'>Â© Mapbox Â© OpenStreetMap</a>", n.length > 2) s = "https://api.mapbox.com/styles/v1/" + n[2].substr(9) + "/tiles/256/{z}/{x}/{y}@2x?access_token=" + this.options.map_access_token;
						else s = "https://api.tiles.mapbox.com/v4/" + n[1] + "/{z}/{x}/{y}.png?access_token=" + this.options.map_access_token;
						i = new L.TileLayer(s, e);
						break;
					case "stadia":
						var a;
						n.length > 1 && (a = n.slice(1).join(":"), this.options.map_access_token && (a = `${a}?api_key=${this.options.map_access_token}`)), i = new L.TileLayer(a, e);
						break;
					case "stamen":
						i = new it(n[1] || "toner-lite", e), this._map.getContainer().style.backgroundColor = "#FFFFFF";
						break;
					case "zoomify":
		...
		class Dt {
			...
			addCallToAction(t) {
				this._el.call_to_action = N.create("div", "vco-slide-calltoaction", this._el.content_container), this._el.call_to_action.innerHTML = "<span class='vco-slide-calltoaction-button-text'>" + t + "</span>", st.addListener(this._el.call_to_action, "click", this._onCallToAction, this)
			}
			_onCallToAction(t) {
		...
		}(Et), T(Et, H), i(16), window.trace = function(t) {
			console.log(t)
		};
		const Ot = {
			Load: {
