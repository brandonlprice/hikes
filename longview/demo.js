let demojs={
	"storymap": {
		"calculate_zoom": false,
		"attribution": "",
		"call_to_action": false,
		"show_history_line":false,
		"zoomify": false,
		"map_type": "https://mt1.google.com/vt/lyrs=y&hl=en&x={x}&y={y}&z={z}",
		"call_to_action_text": "",
		"map_as_image": false,
		"map_subdomains":"xyz",
		"slides": [
			{
			"date": "Jan 2006",
			"background": {         
                    		"url": "images/IMG_9279_11zon.jpg",
                    		"color": "#464646",
                    		"opacity": 99					
                	},
			"media": {
				"url": "https://www.dropbox.com/scl/fi/0azdlydrsnq9xlwcs7kog/longview.mp4?rlkey=qh071e7vmd86x0giosskzw107&st=akua7nih&raw=1",
				"caption": "",
				"credit": ""
			},
			"type": "overview",
			"location": {
				"line": false
			},
			"text": {
				"headline": "Longview Trail to Longview Park<small>Laguna Niguel, CA</small>",
				"text": "This trail travels along the edge of a mansion community in Laguna Niguel. It leads to a small summit with an ocean view called Longview Park. Unfortunately, trail access to the park is blocked. The alternatives, are either to end the hike at another summit before reaching the park, take a makeshift route to the park, or take a different route altogther.<ul style='overflow:hidden;'><li style='float:left;'>3.2 miles (Out & Back)</li><li style='float:left;'>Hillside trail, Pockets of shade, Ocean view</li></ul>"
			}
			},
			{
			"date": "",
			"media": {
				"url": "../salt-creek/images/IMG_7690_11zon.jpg",
				"caption": "Pooch Park Parking Lot",
				"credit": ""
			},
			"location": { 
				"lat":  33.50654184346388, 
				"line": false,
				"lon": -117.68705462808632,
				"zoom": 20
			},
			"text": {
				"headline": "Park in the <br><small><a href='https://maps.app.goo.gl/3uKY3ZWjnizcK67M8' target='_blank'><i>Pooch Park Parking Lot↗</i></a></small>",
				"text": "Park here. It is a free public parking lot for a dog park but also can be used by hikers and cyclists to access the nearby trails."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9230_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50651947859117,    
				"line": false,
				"lon": -117.6869151532177,
				"zoom": 20
			},
			"text": {
				"headline": "Start Here",
				"text": "Start the trail here at this entrance. It is adjoined to the parking lot."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/E37E6E55-54B9-413E-914F-43EB077506C8_11zon.jpeg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50662635213745,   
				"line": false,
				"lon": -117.68624476195777,
				"zoom": 20
			},
			"text": {
				"headline": "Turn Right",
				"text": "Turn right into the tunnel. It goes under Golden Lantern Boulevard."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9233_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 	33.50603639592975, 
				"line": false,
				"lon": -117.68607025737903,
				"zoom": 20
			},
			"text": {
				"headline": "Slight Uphill",
				"text": "You will now be on the other side of the street after exiting the tunnel."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9234_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.505846293217154, 
				"line": false,
				"lon": -117.68659597034531,
				"zoom": 20
			},
			"text": {
				"headline": "Enter Gravel Pathway",
				"text": "Continue onto the gravel path."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9236_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.505325745106575, 
				"line": false,
				"lon": -117.68698489065233,
				"zoom": 20
			},
			"text": {
				"headline": "Veer left",
				"text": "The path will naturally veer left away from the street and change to dirt."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9237_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.505270950386176, 
				"line": false,
				"lon": -117.68614938254414,
				"zoom": 20
			},
			"text": {
				"headline": "Modest Uphill Begins",
				"text": "An incline will ensue for about a third of mile after veering left."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9238_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50514570518051, 
				"line": false,
				"lon": -117.68490483756133,
				"zoom": 20
			},
			"text": {
				"headline": "Uphill Continued",
				"text": "Almost at the halfway point."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9240_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.504987738919944, 
				"line": false,
				"lon": -117.68421565464271,
				"zoom": 20
			},
			"text": {
				"headline": "Uphill Midpoint",
				"text": "This is the halfway point of the modest uphill. The direction of the trail switches going the other way."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9241_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.504807989534456, 
				"line": false,
				"lon": -117.68480559582773,
				"zoom": 20
			},
			"text": {
				"headline": "Hay Cover",
				"text": "The trail may look like it's covered in hay but still should be able to make out."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9242_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50454407837953, 
				"line": false,
				"lon": -117.6853313087944,
				"zoom": 20
			},
			"text": {
				"headline": "Flat Section",
				"text": "The modest uphill is interrupted by this brief flat section followed by the last incline as seen."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9244_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.504130317737356, 
				"line": false,
				"lon": -117.68719007964157,
				"zoom": 20
			},
			"text": {
				"headline": "Pass Fence",
				"text": "Pass the fence. The uphill is over a few feet after passing it."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9245_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50395586670857, 
				"line": false,
				"lon": -117.687041217041,
				"zoom": 20
			},
			"text": {
				"headline": "Enter Neighborhood<br><small>1<sup>st</sup> Gate</small>",
				"text": "Follow the trail. It will take you into the neighborhood where there is an opening in the black gate."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9248_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50359997435057, 
				"line": false,
				"lon": -117.68684876854479,
				"zoom": 20
			},
			"text": {
				"headline": "Straddle Wall",
				"text": "After passing through the gate, the trail straddles a backyard wall."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9249_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.503344446645414, 
				"line": false,
				"lon": -117.68653427953784,
				"zoom": 20
			},
			"text": {
				"headline": "Enter Neighborhood<br><small>2<sup>nd</sup> Gate</small>",
				"text": "Once past the wall, there will be a chain link fence with a swinging gate. Turn right and go through it. It should be open. It is a public access point into the neighborhood and the trail."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9250_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50282024904286, 
				"line": false,
				"lon": -117.68677031393084,
				"zoom": 20
			},
			"text": {
				"headline": "Go Straight",
				"text": "You are now on Longview Trail. A couple rolling hills as seen are in the beginning of the trail."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9255_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50184509327388, 
				"line": false,
				"lon": -117.68660669918093,
				"zoom": 20
			},
			"text": {
				"headline": "Go Straight",
				"text": "Another subtle rolling hill. Glimpses of the ocean may start to appear."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9258_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50066862927307, 
				"line": false,
				"lon": -117.68657183046362,
				"zoom": 20
			},
			"text": {
				"headline": "Veer Right",
				"text": "The trail cuts in between the bushes."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9261_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.5006194232066, 
				"line": false,
				"lon": -117.68684541578283,
				"zoom": 20
			},
			"text": {
				"headline": "Cross Street",
				"text": "The first of three alley crossings."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9262_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50041588870877, 
				"line": false,
				"lon": -117.68703317041373,
				"zoom": 20
			},
			"text": {
				"headline": "Ocean",
				"text": "The ocean is in clear view although still distant."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9264_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.50016538407492, 
				"line": false,
				"lon": -117.68729870910587,
				"zoom": 20
			},
			"text": {
				"headline": "Straddling Wall",
				"text": "The trails abuts the backyard wall."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9266_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49981870235181, 
				"line": false,
				"lon": -117.68733894224178,
				"zoom": 20
			},
			"text": {
				"headline": "Lawn 1",
				"text": "A nicely manicured lawn with a bench."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9272_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.4985068037295, 
				"line": false,
				"lon": -117.68769878096829,
				"zoom": 20
			},
			"text": {
				"headline": "Go Around Bush",
				"text": "Go left around the bush."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9274_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49759543301856, 
				"line": false,
				"lon": -117.68826966877022,
				"zoom": 20
			},
			"text": {
				"headline": "Lawn 2",
				"text": "A less manicured lawn/open space area."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9275_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.497180800387724, 
				"line": false,
				"lon": -117.68849162156657,
				"zoom": 20
			},
			"text": {
				"headline": "Veer Left",
				"text": "A slight decline as you go left around the tree."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9277_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49692581291518, 
				"line": false,
				"lon": -117.68823144729205,
				"zoom": 20
			},
			"text": {
				"headline": "Cross Street",
				"text": "Cross the second alleyway."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9279_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49651755198612, 
				"line": false,
				"lon": -117.68858469442182,
				"zoom": 20
			},
			"text": {
				"headline": "Veer Right",
				"text": "A pocket of trees in the sloping canyon."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9280_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49607691189496, 
				"line": false,
				"lon": -117.68879390672552,
				"zoom": 20
			},
			"text": {
				"headline": "Go Straight",
				"text": "Stay on the dirt path."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9281_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49527391226264, 
				"line": false,
				"lon": -117.6884934993159,
				"zoom": 20
			},
			"text": {
				"headline": "Go Straight",
				"text": "Another subtle rolling hill."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9287_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.494466431574985, 
				"line": false,
				"lon": -117.68816090539825,
				"zoom": 20
			},
			"text": {
				"headline": "Summit 1",
				"text": "A nice view of the ocean under the tree. You could stop here and go back or advance to the next page for an optional extended route that requires scrambling up a hill."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9288_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49411077980483, 
				"line": false,
				"lon": -117.68853105024185,
				"zoom": 20
			},
			"text": {
				"headline": "Cross Street<br><font style='line-height: 1em;' size='6'>Extended Option</font>",
				"text": "Cross the street. This a regular residential street whereas the previous crossings were alleys."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9291_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49413538468737, 
				"line": false,
				"lon": -117.6892793865568,
				"zoom": 20
			},
			"text": {
				"headline": "Check Gate<br><font style='line-height: 1em;' size='6'>Extended Option</font>",
				"text": "Once across the street, this pathway on the left takes you to the second summit. The gate is usually locked though. Check it just in case."
			}
			},
			{
			"date": "",
			"media": {
				"url": "images/IMG_9298_11zon.jpg",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49435682835295, 
				"line": false,
				"lon": -117.68952078536849,
				"zoom": 20
			},
			"text": {
				"headline": "Scramble Uphill<br><font style='line-height: 1em;' size='6'>Extended Option</font>",
				"text": "If it's locked, the only way to the second summit is scrambling up the hill."
			}
			},
			{
			"date": "",
			"media": {
				"url": "https://www.dropbox.com/scl/fi/rhsrjwea2kmqpf5ova5o1/longview_summit.mp4?rlkey=nyqsiyp9r6x5niihg8j9alerm&st=o4yuy753&raw=1",
				"caption": "",
				"credit": ""
			},
			"location": {
				"lat": 33.49384459920669, 
				"line": false,
				"lon": -117.69023693517498,
				"zoom": 20
			},
			"text": {
				"headline": "Longview Park (Summit 2)<br><font style='line-height: 1em;' size='6'>Extended Option</font>",
				"text": "View of Longview Park. The second summit."
			}
			}
		]
	}
  }
